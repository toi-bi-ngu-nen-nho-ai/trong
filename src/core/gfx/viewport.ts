...
// Thay ràng buộc Lit của BlockSuite: thượng nguồn khai
// `import type { GfxViewportElement } from '.'` — `viewport-element.ts` là file Lit, không port ở
// P0-C (React sẽ thay ở P1.0). Đây là `import type` nên không có mã Lit nào chạy thật ở đây.
//
// Đã đo bằng grep trên toàn bộ file gốc: `GfxViewportElement` chỉ xuất hiện ở vị trí kiểu
// (`_element: GfxViewportElement | null`, `elementReady = new Subject<GfxViewportElement>()`) —
// không có chỗ nào truy cập thành viên trên giá trị mang kiểu này (`this._element.<gì đó>`).
// Vì thế kiểu tối giản rỗng là đủ và trung thực, đúng lối `EditorHost` trong `./host`.
import type { GfxViewportElement } from './host';
