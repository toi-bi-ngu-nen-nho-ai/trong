...
- Mindmap (`MindmapScreen`/`MindmapGallery`/`MindmapBoard` trong `src/App.tsx` + `src/components/MindmapBoard.tsx`): nhiều "bảng" (`MindBoard`) độc lập, mỗi bảng có tên/màu/chuyên khoa gắn thẻ tuỳ chọn. Trong một bảng: ghi chú dạng thẻ (`MindNode`, gắn được thẳng tới một bài trong Thư viện), đường nối giữa thẻ (`MindEdge` — loại "quan hệ" thường hoặc "phác đồ/thuật toán" cho chuỗi bước điều trị, có nhãn tuỳ chọn), nét vẽ tay tự do (bút máy/bút chì/bút dạ/băng dính/hình vẽ), và ảnh chèn. Có tìm xuyên suốt mọi bảng cùng lúc, hoàn tác/làm lại, xuất PNG/PDF hoặc sao chép dạng văn bản (outline). Xoá bảng là xoá mềm (vào thùng rác, phục hồi được) trước khi xoá vĩnh viễn. Dữ liệu mindmap lưu ở IndexedDB (khác phần lớn dữ liệu tự thêm khác trong app, vốn dùng localStorage) — đồng bộ bất đồng bộ, vì khối lượng nét vẽ tay có thể lớn.
