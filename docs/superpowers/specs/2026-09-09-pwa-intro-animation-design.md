...
| Font? | **Baloo 2, tự host** | Baloo 2 là biến thể Latin phổ biến nhất, khớp phong cách bo tròn-đậm trong ảnh mẫu. **Sửa lại nhận định lúc brainstorm:** font UI chính của app (Plus Jakarta Sans...) thực ra tải qua CDN `@import` ở `src/index.css:40` — KHÔNG tự host; tự host chỉ tồn tại riêng cho Inter-trên-canvas (`src/board/phong-chu-bang.ts`, lý do rất đặc thù của BlockSuite). Sau khi biết sự thật này, người dùng vẫn chọn tự host cho Baloo 2 (2026-09-09) để intro luôn đúng font kể cả mất mạng ở lần mở đầu — chấp nhận đổi lấy 3 file (~70KB, đã xác minh qua HEAD request) + một bước xin phép tải. |
