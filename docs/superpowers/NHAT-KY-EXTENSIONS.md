# Nhật ký bật/tắt view extension của cây BlockSuite

Tách khỏi khối chú thích đầu `src/board/extensions.ts` ngày 2026-09-05 (khoản Minor #15 của
review toàn nhánh page mode). Khối ấy đã dài hơn 260 dòng vì §0 luật 5 của plan bắt ĐO và GHI
số liệu sau MỖI nhóm extension được bật — đúng quy trình, nhưng dồn hết vào đầu một tệp mã thì
phần đang-hiệu-lực bị chôn dưới phần lịch sử.

**Phần đang-hiệu-lực vẫn nằm ở `src/board/extensions.ts`**: con số 55/58, ba mục còn thiếu và lý
do của từng mục, cùng luật thứ tự widget ảnh hưởng z-index. Tệp này giữ phần còn lại: vì sao
từng chặng bật thêm cái gì, đo được bao nhiêu, và những đường đã thử rồi phải gỡ.

Nội dung dưới đây chép nguyên văn từ khối chú thích cũ, chỉ bỏ tiền tố `// ` và sửa những cụm trỏ
vị trí mà chính phép tách này làm sai ("ở đầu file" → trỏ rõ sang `src/board/extensions.ts`).
...
nào trong bốn nhóm page mode; xem giải thích ở đầu `src/board/extensions.ts`.
