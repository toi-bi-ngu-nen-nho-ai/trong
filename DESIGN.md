...
- **Favorite Gold** (`--c-fav` / `--c-fav-soft`, `#a16207` + `#fdf3d6` light / `#facc15` + `#2c2411` dark): the *only* warm hue in the app, scoped to exactly one control — the "pin group to the front of the tab row" star toggle on `DungThuocScreen` (`icons.starPin`, both the tab-row instance and the search-panel jump list). Added 2026-09-03 at the owner's explicit direction ("the favorite star must be yellow"). A filled gold star is the universal favorite/bookmark affordance; the earlier indigo-primary version was system-consistent but had lost that instant recognizability. **This is a deliberate, scoped exception to the cool-violet system** — same "One Other Place Rule" as Mindmap Magenta and Note Paper, scoped to a single 14px glyph plus a 1px border hairline so it never reads as a warm stain in the tab row. It is **not** part of the `--c-warn*` family and must never be: the hue is pulled deliberately toward yellow-gold and away from the clinical amber (`--c-warn-icon` is `#b45309` rust in light, `#f0b429` orange-amber in dark) so the two never read as the same signal, and the star lives in the tab-row chrome, never on a dose or safety surface. Light mode is forced to a dark gold because bright yellow cannot clear 3:1 as a filled mark on a light ground; dark mode gets the bright lemon-gold. Off state is a hollow (outline) star in `--c-text-soft` on `--c-line-soft`; on state is the filled gold star on `--c-fav-soft` with a `--c-fav` border — four redundant cues (fill, outline, background, border), never opacity alone.
