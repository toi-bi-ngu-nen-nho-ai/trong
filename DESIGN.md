...
colors:
  primary: "#1c79f2"
  primary-strong: "#0e6eec"
  primary-deep: "#0b55b7"
  primary-soft: "#edf1f8"
  primary-line: "#c0d5f2"
  primary-line-2: "#93b9ec"
  on-primary: "#121212"
  accent-2: "#b8196f"
  danger: "#b91c1c"
  danger-icon: "#dc2626"
...
**Creative North Star: "The Bedside Precision Panel" — now in Facebook Blue**

Every screen exists to answer one question fast and correctly, then get out of the way. The system is token-driven end to end — every color in the app reads from a `--c-*` CSS custom property, never a hardcoded hex — so a single palette swap recolors the entire product without touching component code. That architecture is itself a design principle: it makes the system honest about what's decoration and what's load-bearing. It's also what made all three rebrands (2026-08, Deep Azure → Electric Indigo; 2026-09 first pass, Electric Indigo → AFFiNE Blue; 2026-09 second pass, AFFiNE Blue → Facebook Blue) token-file changes rather than component-by-component rewrites.

The palette is **Facebook Blue** (`#1c79f2` light / `#5697eb` dark, hue ≈214°) — the third rebrand of this app. The neutral gray scale (true gray, hue=0) still traces back to AFFiNE's own theme tokens from the first 2026-09 pass; only the brand hue itself moved a second time, same day, after the owner compared the AFFiNE hue (≈205°) side-by-side with well-known brand blues and found it read as "the same family as Facebook/Twitter" anyway — so the app committed to that family deliberately (Facebook's actual `#1877F2`, hue ≈214°) rather than sit in an ambiguous in-between. Light mode uses the hue at brand-accurate saturation and lightness (nearly `#1877F2` itself); dark mode is a *lighter tint of the same hue*, not the "nearly identical blue in both themes" relationship AFFiNE happens to use — see Primary below for why that relationship doesn't survive a hue this far into blue. The system deliberately draws a hard line between two zones: **decoration** (navigation, tabs, cards, screen transitions) and **diagnosis** (dose numbers, safety-tier colors, warning language). Delight — spring/bounce motion, a livelier accent color — lives entirely in the first zone. The second zone is untouchable: red always means danger, amber always means caution, and neither one bounces, glows, or competes with a decorative accent for attention. This isn't timidity; it's the system's one non-negotiable rule, confirmed directly against real clinical stakes rather than assumed, and it survived all three rebrands untouched — the original reason indigo was chosen over a green/teal candidate (and the reason both blue successors were acceptable) was to keep the brand hue maximally distinct from all three safety hues, not just red and amber.
...
The palette reads as one confident, cool-blue system: Facebook Blue as the single brand color, a true neutral gray (hue=0, 0% saturation) for every surface and border, and a fully separate, untouched hazard-signal family for clinical safety states.

### Primary
- **Facebook Blue** (`#1c79f2` light mode / `#5697eb` dark mode, hue ≈214°): the app's one brand color. Light mode is Facebook's real brand blue `#1877F2` nudged one HSL-lightness point (52%→53%) so `--c-on-primary` clears AA — a deviation too small to see. Used for solid fills, borders, and focus rings (primary buttons, active tab/chip fills, the checkbox check, `--ring`) — anywhere the WCAG non-text 3:1 floor applies, not the 4.5:1 text floor: `#1c79f2` alone only clears **4.14:1** as text/icon color on `--c-surface`, so it is *not* safe as body-sized text on a light surface — see `--c-primary-strong` below. Dark mode is a **lighter tint of the same hue** (`#5697eb`, L=63% vs light's L=53%) rather than "nearly the same blue in both themes": at this hue (closer to blue, farther from cyan than the AFFiNE hue this replaced), WCAG relative luminance is markedly lower at a given lightness — the formula weights the blue channel at only 0.0722 versus green's 0.7152 — so holding the light-mode lightness in dark mode would read too dark against `--c-page` and fail AA as text. `#5697eb` clears **6.16:1** on `--c-page`.
- **Primary Strong** (`#0e6eec` light / `#84b3f1` dark): the text/icon-safe tier — 4.71:1 on `--c-surface` in light mode. Every tab label, link, and small primary-colored icon on a light surface must use this instead of raw `--c-primary`; the fill of a button or chip still uses full-strength `--c-primary`. (On `--c-page` rather than a card, check `-strong` again before assuming it clears AA there — see Primary Deep.) In dark mode `-strong` isn't an AA fix — base `--c-primary` already clears AA there — it's simply a brighter emphasis step above the base.
- **Primary Deep** (`#0b55b7` light / `#a4c7f4` dark): the highest-emphasis tier, 7.01:1 on `--c-surface` — used where `-strong` still isn't enough, e.g. primary-colored text sitting directly on `--c-page`.
- **Primary Soft** (`#edf1f8` light / `#16263b` dark): background tint for icon badges and secondary surfaces that want to feel "in the brand" without full-strength color.
- **Primary Line** (`#c0d5f2` light / `#2d4d76` dark): border/accent-line weight, one step down from full-strength primary. **Primary Line 2** (`#93b9ec` light / `#335f99` dark) is one step further still — same hue, one HSL-lightness step darker (light) / lighter (dark) than `-line`.
- **On Primary** (`--c-on-primary`, `#121212`, fixed in both themes — does not flip like `--c-on-bright`): the ink for text/icons drawn on a *solid* `--c-primary` fill (a primary button's label, an active tab chip's text). White text on either theme's primary fails or barely clears AA at this hue, so this ink stays dark. It happens to clear AA in *both* themes (4.52:1 light, 6.26:1 dark) despite light and dark no longer sharing nearly the same lightness — that's a coincidence of these particular numbers, not a rule; re-verify if either primary tier changes again.
